...
class MeanMaxPooling(nn.Module):
    def __init__(self, config):
        ...
        self.linear_layer = nn.Linear(config.hidden_size *2 , config.hidden_size *2)

    def forward(self, x : torch.Tensor) -> torch.Tensor:
        mean_pooling_embeddings = torch.mean(x, 1)
        max_pooling_embeddings, _ = torch.max(x, 1)
        # _, max_pooling_embeddings = torch.max(x, 1)

        # torch.max(x,1) -> 1번째 차원으로 max하는데, [0]이 value고 [1]이 indices -> [0]으로해야함
        # 이거때문에 잘못돼서 평균f1 0.7~8나왔었음
        # max_pooling_embeddings = torch.max(x, 1)[0]

        mean_max_embeddings = torch.cat((mean_pooling_embeddings, max_pooling_embeddings), 1)

        # output = self.dense(mean_max_embeddings)  # 성능별로라 이거 안쓰기로함
        # output = self.activation(output)          # 성능별로라 이거 안쓰기로함
        output = self.dropout(mean_max_embeddings)
        # output = self.dense(output)               # 성능별로라 이거 안쓰기로함
        # output = torch.tanh(output)               # 성능별로라 이거 안쓰기로함
        # output = self.dropout(output)             # 성능별로라 이거 안쓰기로함
...
# hiddendim_lstm에 대한 기준이 애매하다.. 일단 256으로했는데 512로 실험은 못해봄
class LSTMPooling(nn.Module):
    def __init__(self, num_layers, hidden_size, hiddendim_lstm,config):
        ...
        self.linear_layer = nn.Linear(self.hiddendim_lstm , config.hidden_size)
...
class WeightedLayerPooling(nn.Module):
    def __init__(self, config, num_hidden_layers, layer_start: int = 4, layer_weights = None):
